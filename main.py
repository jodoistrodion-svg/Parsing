...
BUY_TIMEOUT = float((os.getenv("BUY_TIMEOUT") or "0.45").strip())
...
AUTOBUY_MAX_HTTP_ATTEMPTS = int((os.getenv("AUTOBUY_MAX_HTTP_ATTEMPTS") or "12").strip())
AUTOBUY_MAX_DURATION_SEC = float((os.getenv("AUTOBUY_MAX_DURATION_SEC") or "1.10").strip())
...
def _autobuy_speed_estimate_text() -> str:
    # Оценка времени первого/полного цикла на основании текущих env-настроек.
    # Это не SLA, а ориентир для деплоя/тюнинга под текущую сеть.
    first_wave_sec = max(FAST_AUTOBUY_TIMEOUT, BUY_TIMEOUT)

    attempts = AUTOBUY_RETRY_ATTEMPTS if AUTOBUY_RETRY_ATTEMPTS > 0 else 1
    if AUTOBUY_MAX_DURATION_SEC > 0:
        estimated_full_sec = min(AUTOBUY_MAX_DURATION_SEC, first_wave_sec * attempts)
        budget_label = f"<= {AUTOBUY_MAX_DURATION_SEC:.2f}s"
    else:
        estimated_full_sec = first_wave_sec * attempts
        budget_label = "unbounded"

    return (
        f"fast~{first_wave_sec:.2f}s | full~{estimated_full_sec:.2f}s "
        f"(attempts={attempts}, budget={budget_label}, http_limit={AUTOBUY_MAX_HTTP_ATTEMPTS})"
    )
...
async def show_status(user_id: int, chat_id: int):
    ...
    autobuy_speed = _autobuy_speed_estimate_text()
    ...
    text = (
        ...
        f"• Скорость автобая (оценка): <code>{html.escape(autobuy_speed)}</code>\n"
...
async def main():
    ...
    print(f"[BOT] Autobuy speed estimate: {_autobuy_speed_estimate_text()}")
    log_autobuy(f"DEPLOY_AUTOBUY_PROFILE {_autobuy_speed_estimate_text()}")
