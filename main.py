...
AUTOBUY_TOTAL_RETRY_WINDOW_SEC = float((os.getenv("AUTOBUY_TOTAL_RETRY_WINDOW_SEC") or "0").strip())
...
async def try_autobuy_item(source: dict, item: dict, found_perf: float | None = None):
    ...
    async with lock:
        configured_attempts = AUTOBUY_RETRY_ATTEMPTS
        finite_attempts = configured_attempts > 0
        attempts = configured_attempts if finite_attempts else 0
        i = 0
        ...
        while True:
            ...
            if remaining_window is not None and remaining_window <= 0:
                total = str(attempts) if finite_attempts else "∞"
                return False, f"attempt={i-1}/{total} | autobuy_total_window_exceeded"
            ...
            if finite_attempts and i >= attempts:
            ...
            if delay > 0:
                remaining_window = _remaining_autobuy_window_sec(found_perf)
                if remaining_window is not None and remaining_window <= 0:
                    return False, f"attempt={i}/{total} | autobuy_total_window_exceeded"
